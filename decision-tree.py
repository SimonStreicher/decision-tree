...
def compute_tree(dataset, parent_node, classifier):
    ...
    for attr_index in range(len(dataset.examples[0])):
        ...
        if dataset.attributes[attr_index] != classifier:
            ...
            attr_value_list = [
                example[attr_index] for example in dataset.examples
            ...
            for val in attr_value_list:
                ...
                local_gain = calc_gain(
                    dataset, dataset_entropy, val, attr_index
                )  # calculate the gain if we split on this value
    ...
    if (split_val is None) or (attr_to_split is None):
        print(
            "Something went wrong. Couldn't find an attribute to split on or a split value."
        )
...
def calc_gain(dataset, entropy, val, attr_index):
    ...
    if (
        len(gain_upper_dataset.examples) == 0 or len(gain_lower_dataset.examples) == 0
    ):  # Splitting didn't actually split (we tried to split on the max or min of the attribute's range)
        return -1

    attr_entropy += (
        calc_dataset_entropy(gain_upper_dataset, classifier)
        * len(gain_upper_dataset.examples)
        / total_examples
    )
    attr_entropy += (
        calc_dataset_entropy(gain_lower_dataset, classifier)
        * len(gain_lower_dataset.examples)
        / total_examples
    )
...
def main():
    ...
    if len(args) < 2:
        ...
        if "-v" in args:
            ...
            print(
                "Initial (pre-pruning) validation set score: "
                + str(100 * best_score)
                + "%"
            )
        if "-p" in args:
            if "-v" not in args:
                ...
                post_prune_accuracy = 100 * prune_tree(
                    root, root, validateset, best_score
                )
                print(
                    "Post-pruning score on validation set: "
        ...
        if "-t" in args:
            ...
            a = csv.writer(b)
            for example in testset.examples:
                example[testset.class_index] = test_example(
                    example, root, testset.class_index
                )
            saveset = testset
